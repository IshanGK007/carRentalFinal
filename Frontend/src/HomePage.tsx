...
const HomePage = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [carImages, setCarImages] = useState({}); // Store images in state
  const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetchCarData = async () => {
      ...
      try {
        ...
        // Fetch car images for each car
        const images = {};
        for (const car of data) {
          const imageUrl = await fetchCarImage(car.imageId);
          images[car._id] = imageUrl; // Map car id to image URL
        }
        setCarImages(images); // Store the image URLs
      } catch (error) {
        console.error("Error fetching car data:", error);
      } finally {
        setLoading(false);
  ...
  const fetchCarImage = async (imageId) => {
    try {
      ...
      return "https://via.placeholder.com/400x250"; // Fallback image URL
    }
  };
      ...
      <div className="max-w-7xl mx-auto py-8 px-4">
        <div className="bg-gray-50 shadow-lg rounded-xl flex flex-col md:flex-row items-center">
          ...
          <div className="p-6">
            <h3 className="text-2xl font-semibold mb-4">Mahindra Thar</h3>
            <p className="text-gray-600 mb-4">Rugged. Reliable. Resilient.</p>
            <button className="bg-black text-white px-6 py-2 rounded-lg hover:bg-gray-800" onClick={() => handleViewDeal("67760e4687eab1a8949dd920")}>
              Learn More
            </button>
          </div>
        </div>
            ...
            {carData[selectedCategory]?.length === 0 ? (
              ...
              carData[selectedCategory].map((car) => (
                <div
                  ...
                  className="relative bg-white shadow-xl rounded-lg overflow-hidden transition-all transform hover:scale-105 hover:shadow-2xl"
                >
                  {/* Fetch car image dynamically */}
                  <img
                    src={carImages[car._id] || "https://via.placeholder.com/400x250"} // Use fetched image URL or fallback image
                    alt={car.make + " " + car.model}
                    className="w-full h-48 object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-40"></div>
                  ...
                  <div className="p-6 relative z-10">
                    <h3 className="text-xl font-semibold text-gray-800">{car.make} {car.model}</h3>
                    <p className="text-gray-500 text-sm">Price: ₹{car.price} per month</p>
                    <div className="flex items-center space-x-2 mt-2">
                      <span className="text-yellow-500">
                        {'★'.repeat(Math.round(car.rating))}
                      </span>
